...
namespace caffe
{
	...
	void Similarity3LossLayer<Dtype>::LayerSetUp(
	...
	template <typename Dtype>
	void Similarity3LossLayer<Dtype>::Reshape(
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
	...
	template <typename Dtype>
	void Similarity3LossLayer<Dtype>::Forward_cpu(
		const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top)
	{
		LOG(FATAL) << "similarity3 loss layer cannot run with cpu";
	...
	template <typename Dtype>
	void Similarity3LossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	#ifdef CPU_ONLY
	STUB_GPU(Similarity3LossLayer);
	#endif

	INSTANTIATE_CLASS(Similarity3LossLayer);
	REGISTER_LAYER_CLASS(Similarity3Loss);
}