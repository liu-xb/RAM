...
#include "caffe/layers/normalization_l2_layer.hpp"

namespace caffe {
    
    template <typename Dtype>
    void NormalizationL2Layer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    template <typename Dtype>
    void NormalizationL2Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                                const vector<Blob<Dtype>*>& top) {
    ...
    template <typename Dtype>
    void NormalizationL2Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
    STUB_GPU(NormalizationL2Layer);
#endif
    
    INSTANTIATE_CLASS(NormalizationL2Layer);
    REGISTER_LAYER_CLASS(NormalizationL2);
