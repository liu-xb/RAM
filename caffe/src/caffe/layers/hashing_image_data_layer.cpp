...
void HashingImageDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int cat_id = 0; cat_id < cat_per_iter; cat_id++) {
	  ...
	  for (int item_id_xb = 0; item_id_xb < item_per_cat; ++item_id_xb) {
		  // std::cout << lines_[idx[category[cat_id]][item_id]].first << lines_[idx[category[cat_id]][item_id]].second[0] << std::endl;
		  // get a blob
		  timer.Start();
      int item_id=0;
      item_id = item_id_xb % idx[category[cat_id]].size();
      // LOG(ERROR)<<item_id_xb<<" "<<item_id<<" "<<idx[category[cat_id]].size();
		  ...
		  int offset = batch->data_.offset( cat_id * item_per_cat + item_id_xb);
		  this->transformed_data_.set_cpu_data(prefetch_data + offset);
		  this->data_transformer_->Transform(cv_img, &(this->transformed_data_));
		  trans_time += timer.MicroSeconds();
		  prefetch_label[cat_id * item_per_cat + item_id_xb] = lines_[idx[category[cat_id]][item_id]].second[0];
	  }
  }
  batch_timer.Stop();
