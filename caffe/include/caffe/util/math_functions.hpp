...
void caffe_gpu_max(int n, const Dtype* x, int* result);

template <typename Dtype>
void caffe_gpu_min(int n, const Dtype* x, int* result);

template <typename Dtype>
void lfcl_cpu_gemm(const int M, const int N, const int K, const int C,
    const Dtype alpha, const Dtype* bottom, const Dtype* weight, const Dtype beta,
    Dtype* top);

template <typename Dtype>
void lfcl_gpu_gemv(const int M, const int N, const Dtype alpha, 
    const Dtype* A, const Dtype* x, const Dtype beta, Dtype* y);

template <typename Dtype>
void sll_cpu_nrm2(const int code_length, const int batch_size,
  const Dtype* bottom, Dtype* norm_vec);  // to be used in similarity loss layer .cpp

template <typename Dtype>
void lfcl_gpu_gemm(const int M, const int N, const int K, const int C,
    const Dtype alpha, const Dtype* bottom, const Dtype* weight, const Dtype beta,
    Dtype* top);
...
void lfcl_gpu_gemmBatched_backward_bottom(const int M, const int N, const int K, const int C,
    const Dtype* alpha, const Dtype* weight[], const Dtype* top[], const Dtype* beta,
    Dtype* bottom[]);

template <typename Dtype>
void sll_gpu_nrm2(const int code_length, const int batch_size,
  const Dtype* bottom, Dtype* norm_vec);  // to be used in similarity loss layer .cu
